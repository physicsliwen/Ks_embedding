...
void reconstruct_mcv0(TString InputFileList, Int_t Nlist, Int_t Block, Int_t nEvents, TString OutputDir) 
{
  ...
  mcsig -> setHistoFileName(OutputDir+Name+".ks.histo.root") ; // Name the output file for histograms
  mcsig -> setV0TreeFileName(OutputDir+Name+".ks.picodst.root"); // V0 candidate tree file for further cuts. 
  mcsig -> setV0Type(kKs);   //set V0 type. kKs, or kLambda, kAntiLambda. once a time! 
  mcsig -> SetDebug(0);
